...
function _runRobot(state, robot, memory) {
  for (let turn = 0;; turn++) {
    if (state.parcels.length == 0) {
      return turn;
    }
...
function compareRobots(robot1, memory1, robot2, memory2) {
  ...
  for (let i=0; i<times; i++) {
    ...
    turns1 += _runRobot(state, robot1, memory1);
    turns2 += _runRobot(state, robot2, memory2);
  }
...
function _yourRobot({place, parcels}, route) {
  let shortest_length = Infinity;
  let shortest_route = route;
  if (route.length == 0) {
    for (let parcel of parcels) {
